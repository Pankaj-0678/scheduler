...
def _load_demo_data():
    ...
    st.session_state.rooms = [
        ...
        Room("Lec3", 60, "Lecture"), Room("Lec4", 60, "Lecture"),
        Room("Lec5", 60, "Lecture"), Room("Lec6", 60, "Lecture"),
        Room("Lab1", 30, "Lab"), Room("Lab2", 30, "Lab"), 
        Room("Lab3", 30, "Lab"), Room("Lab4", 30, "Lab"),
        Room("Lab5", 30, "Lab"), Room("Lab6", 30, "Lab")
    ]
    
    # Increased to 16 Instructors to prevent LY Lab overload
    st.session_state.instructors = [Instructor(f"T{i}", f"Prof_{i}", max_hours=18) for i in range(1, 17)]
    
    c_math = Course("C_MATH", "Math-III", 60, [st.session_state.instructors[0], st.session_state.instructors[1]], 3, "Lecture")
    c_dsa = Course("C_DSA", "Data Structures", 60, [st.session_state.instructors[2], st.session_state.instructors[3]], 3, "Lecture")
    l_dsa = Course("L_DSA", "DSA Lab", 30, [st.session_state.instructors[2], st.session_state.instructors[3]], 1, "Lab")
    c_dbms = Course("C_DBMS", "DBMS", 60, [st.session_state.instructors[4], st.session_state.instructors[5]], 3, "Lecture")
    l_dbms = Course("L_DBMS", "DBMS Lab", 30, [st.session_state.instructors[4], st.session_state.instructors[5]], 1, "Lab")
    
    c_os = Course("C_OS", "Operating Systems", 60, [st.session_state.instructors[6], st.session_state.instructors[7]], 3, "Lecture")
    l_os = Course("L_OS", "OS Lab", 30, [st.session_state.instructors[6], st.session_state.instructors[7]], 1, "Lab")
    c_cn = Course("C_CN", "Computer Networks", 60, [st.session_state.instructors[8], st.session_state.instructors[9]], 3, "Lecture")
    l_cn = Course("L_CN", "CN Lab", 30, [st.session_state.instructors[8], st.session_state.instructors[9]], 1, "Lab")
    
    m_iot = Course("M_IOT", "IoT Minor", 60, [st.session_state.instructors[10]], 2, "Minor")
    ml_iot = Course("ML_IOT", "IoT Minor Lab", 30, [st.session_state.instructors[10]], 1, "Minor Lab")
    m_bio = Course("M_BIO", "BioMedical Minor", 60, [st.session_state.instructors[11]], 2, "Minor")
    ml_bio = Course("ML_BIO", "BioMedical Minor Lab", 30, [st.session_state.instructors[11]], 1, "Minor Lab")
    
    c_cloud = Course("C_CLD", "Cloud Computing", 60, [st.session_state.instructors[12]], 3, "Lecture")
    l_cloud = Course("L_CLD", "Cloud Lab", 30, [st.session_state.instructors[12], st.session_state.instructors[13]], 2, "Lab")
    c_ai = Course("C_AI", "Art. Intelligence", 60, [st.session_state.instructors[14], st.session_state.instructors[15]], 3, "Lecture")
    l_ai = Course("L_AI", "AI Lab", 30, [st.session_state.instructors[14], st.session_state.instructors[15]], 2, "Lab")
    
    st.session_state.courses = [c_math, c_dsa, l_dsa, c_dbms, l_dbms, c_os, l_os, c_cn, l_cn, m_iot, ml_iot, m_bio, ml_bio, c_cloud, l_cloud, c_ai, l_ai]
    
    sec_sy_a = DepartmentSection("SY_A", "SY-Div A", 60, 4)
    sec_sy_b = DepartmentSection("SY_B", "SY-Div B", 60, 4)
    sec_ty_a = DepartmentSection("TY_A", "TY-Div A", 60, 4)
    sec_ty_b = DepartmentSection("TY_B", "TY-Div B", 60, 4)
    sec_ly_a = DepartmentSection("LY_A", "LY-Div A", 60, 4)
    
    st.session_state.sections = [sec_sy_a, sec_sy_b, sec_ty_a, sec_ty_b, sec_ly_a]
    
    st.session_state.section_courses = {
        sec_sy_a.id: [c_math, c_dsa, l_dsa, c_dbms, l_dbms],
        sec_sy_b.id: [c_math, c_dsa, l_dsa, c_dbms, l_dbms],
        sec_ty_a.id: [c_os, l_os, c_cn, l_cn, m_iot, ml_iot, m_bio, ml_bio],
        sec_ty_b.id: [c_os, l_os, c_cn, l_cn, m_iot, ml_iot, m_bio, ml_bio],
        sec_ly_a.id: [c_cloud, l_cloud, c_ai, l_ai]
    }
    
    m_iot.minor_batches[sec_ty_a.id] = ["B1", "B2"]
    ml_iot.minor_batches[sec_ty_a.id] = ["B1", "B2"]
    m_bio.minor_batches[sec_ty_a.id] = ["B3", "B4"]
    ml_bio.minor_batches[sec_ty_a.id] = ["B3", "B4"]
    
    m_iot.minor_batches[sec_ty_b.id] = ["B1", "B2"]
    ml_iot.minor_batches[sec_ty_b.id] = ["B1", "B2"]
    m_bio.minor_batches[sec_ty_b.id] = ["B3", "B4"]
    ml_bio.minor_batches[sec_ty_b.id] = ["B3", "B4"]
    
    m_iot.enrolled_sections = [sec_ty_a.id, sec_ty_b.id]
    ml_iot.enrolled_sections = [sec_ty_a.id, sec_ty_b.id]
    m_bio.enrolled_sections = [sec_ty_a.id, sec_ty_b.id]
    ml_bio.enrolled_sections = [sec_ty_a.id, sec_ty_b.id]

    st.success("✅ Full Department Data Loaded! (SY, TY, LY configured with Minors & Strict Isolation)")
                ...
                def _parse_cell(cell_text: str, div_name: str):
                    ...
                    for raw in events:
                        ...
                        if t_name not in instructors_dict:
                            instructors_dict[t_name] = Instructor(f"T{len(instructors_dict)+1}", t_name, max_hours=18)
...
with tab_resources:
    ...
    with col2:
        ...
        with st.form("teacher_form", clear_on_submit=True):
            ...
            t_max_hours = st.number_input("Max hours (classes) per week", 1, 60, 18)
